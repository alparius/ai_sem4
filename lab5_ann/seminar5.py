...
class Network:
    ...
    def errorsBackpropagate(self, err):
        for layer in range(self.noHiddenLayers + 1, 1, -1):
            i = 0
            for n1 in self.layers[layer].neurons:  # i added .neurons
                if layer == self.noHiddenLayers + 1:
                    n1.err(err[i])
                else:
                    sumErr = 0.0
                    for n2 in self.layers[layer + 1].neurons:  # i added .neurons
                        sumErr += n2.weights[i] * n2.err
                    n1.err = sumErr
                for j in range(n1.noInputs):
                    netWeight = n1.weights[j] + LEARN_RATE * n1.err() * self.layers[layer - 1].neurons[j].output
